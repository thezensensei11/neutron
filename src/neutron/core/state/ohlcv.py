...
class OHLCVStateManager(StateManager):
    """
    Manages the state of downloaded OHLCV data.
    """
    def __init__(self, state_file: str = "ohlcv_data_state.json"):
    ...
    def get_gaps(
        ...
        state = self.load()
        ranges = state.get(exchange, {}).get(instrument_type, {}).get(symbol, {}).get(timeframe, [])
    ...
    def update_state(
        ...
        def updater(state):
            ...
            if timeframe not in state[exchange][instrument_type][symbol]: state[exchange][instrument_type][symbol][timeframe] = []

            ranges = state[exchange][instrument_type][symbol][timeframe]
            intervals = []
            for r in ranges:
                s = datetime.fromisoformat(r[0])
                e = datetime.fromisoformat(r[1])
            ...
            state[exchange][instrument_type][symbol][timeframe] = new_ranges
